...
# point30_normalized = (1.0, 0.0)
# point11_normalized = (0.7071067811865475, 0.7071067811865475)
...
# point30_length_5 = (5.0, 0.0)
# point11_length_10sqrt2 = (10.0, 10.0)

# copy()复制一个二维点，或者二维向量，以 (1,1) 为例
point11_copid = point11.copy()
print("point11_copid =", point11_copid)
# point11_copid = (1.0, 1.0)
...
# point30_add_11 = (4.0, 1.0)
# point30_add_11_another_way = (4.0, 1.0)
...
# point30_neg = (-3.0, -0.0)
# point30_neg_another_way = (-3.0, -0.0)
...
# point30_sub_11 = (2.0, -1.0)
# point30_sub_11_another_way = (2.0, -1.0)
...
# 原地相加前 point30_for_iadd1= (3.0, 0.0)
# 原地相加前 point30_for_iadd2= (3.0, 0.0)
# 原地相加后 point30_for_iadd1= (4.0, 1.0)
# 原地相加后 point30_for_iadd2= (4.0, 1.0)
...
# 原地相减前 point30_for_isub1= (3.0, 0.0)
# 原地相减前 point30_for_isub2= (3.0, 0.0)
# 原地相减后 point30_for_isub1= (2.0, -1.0)
# 原地相减后 point30_for_isub2= (2.0, -1.0)

# 矢量旋转相关
# _rotation_matrix(phi) 获取旋转矩阵，这是一个 2*2 的矩阵。注意 phi 单位为弧度
# 以 30 度为例，使用 BaseUtils.angle_to_radian(30) 将角度制转为弧度制
...
# 旋转矩阵 30 度 [[0.8660254037844387, -0.49999999999999994], [0.49999999999999994, 0.8660254037844387]]
# 旋转矩阵 45 度 [[0.7071067811865476, -0.7071067811865476], [0.7071067811865476, 0.7071067811865476]]
...
# matrix1010 × (3,4) (3.0, 4.0)
# matrix1000 × (3,4) (3.0, 0.0)
# matrix0001 × (3,4) (0.0, 4.0)

# rotate(phi)，矢量旋转，正角表示逆时针旋转，返回新矢量，以 (1,0) 为例
point10 = P2(1, 0)
...
# (1,0) 逆时针旋转 30 度 (0.8660254037844387, 0.49999999999999994)
# (1,0) 逆时针旋转 45 度 (0.7071067811865476, 0.7071067811865476)
...
# point10_rotate30_angle_to_x_axis = 29.999999999999996
# point10_rotate45_angle_to_x_axis = 45.0
...
print("(3,4).to_p3() =",point34.to_p3()) # (3.0, 4.0, 0.0)
print("(3,4).to_p3(lambda) =",point34.to_p3(lambda p2:P3(p2.x,p2.y,p2.x+p2.y))) # (3.0, 4.0, 7.0)
...
# 若 msg 指定，则当判断结果为相等时，返回 True，判断结果为不相等时，报出异常信息
point10_add_1e6 = P2(1+1e-6,0)
point10_add_2e6 = P2(1+2e-6,0)
print(point10==point10_add_1e6) # True
print(point10==point10_add_2e6) # False

try:
    point10.__eq__(3)
except ValueError as ve:
    print("遇到异常：",ve)
    # 遇到异常： 3 不是 P2 不能进行相等判断
else:
...
try:
    ...
    print("遇到异常：",ae)
    # 遇到异常： (1.0, 0.0)和(1.000002, 0.0)不相等
else:
    raise AssertionError("居然没有异常")
...
print(P2.from_numpy_ndarry(ndarray12)) # (1.0, 2.0)
print(P2.from_numpy_ndarry(ndarray_1_2)) # (1.0, 2.0)
print(P2.from_numpy_ndarry(ndarray12_34)) # [(1.0, 2.0), (3.0, 4.0)]

try:
    print(P2.from_numpy_ndarry(ndarray123))
except Exception as e:
    print("遇到异常：",e)
    # 遇到异常： 无法将[1 2 3]转为P2或List[P2]
