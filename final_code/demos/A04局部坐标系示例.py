...
# 查看坐标
print(point_lcs_233) # (0.0, 1.0, 2.0)


# 函数 point_to_global_coordinate(local_coordinate_point) 将局部坐标系表示的点 local_coordinate_point 转为全局坐标
print(lcs221.point_to_global_coordinate(point_lcs_233)) # (2.0, 3.0, 3.0)
...
print(vector_gcs_233,vector_lcs_233) # (2.0, 3.0, 3.0) (2.0, 3.0, 3.0)

# 函数 __str__() 和 __repr__() 将坐标系转为字符串
# 分别打印局部坐标系的原点、xyz三个轴方向在全局坐标系的坐标
# 下面三个打印结果相同 LOCATION=(2.0, 2.0, 1.0), xi=(1.0, 0.0, 0.0), yi=(0.0, 1.0, 0.0), zi=(0.0, 0.0, 1.0)
print(lcs221)
...
print(lcs221==lcs221_little_change) # True
...
print(lcs_created_by_y_and_z_direction) # LOCATION=(1.0, 2.0, 3.0), xi=(0.0, 0.0, 1.0), yi=(1.0, 0.0, 0.0), zi=(0.0, 1.0, 0.0)
...
try:
    ...
    print("抓住异常：",e) # 住异常： 创建 LocalCoordinateSystem 对象异常，x_direction(1.0, 0.0, 0.0)和z_direction(1.0, 0.0, 0.0)不正交
# 2. 创建坐标系时，传入的两个方向会自动归一化
lcs = LocalCoordinateSystem(x_direction=P3(x=2),z_direction=P3(z=3))
print(lcs) # LOCATION=(0.0, 0.0, 0.0), xi=(1.0, 0.0, 0.0), yi=(0.0, 1.0, 0.0), zi=(0.0, 0.0, 1.0)
